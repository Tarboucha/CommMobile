...
-- CreateEnum
CREATE TYPE "booking_status" AS ENUM ('pending', 'confirmed', 'in_progress', 'ready', 'completed', 'cancelled', 'refunded');
...
-- CreateEnum
CREATE TYPE "offering_category" AS ENUM ('product', 'service', 'share', 'event');
...
CREATE TABLE "schedule_instances" (
    ...
    CONSTRAINT "schedule_instances_pkey" PRIMARY KEY ("schedule_id","instance_date")
);
...
-- AddForeignKey
ALTER TABLE "profiles" ADD CONSTRAINT "profiles_auth_user_id_fkey" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE ON UPDATE NO ACTION;
